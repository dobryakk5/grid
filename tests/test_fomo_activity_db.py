...
from app.db.models import Base, FomoActivityLeg, FomoLeaderboardState
    ...
    try:
        async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://test") as http:
            ...
            payload = {"requested_limit": 1, "traders": [{
                "user_id": "alice", "handle": "alice", "rank": 1, "has_more": True, "swaps": [swap],
            }]}
            for _ in range(2):
            ...
            assert len((await http.get("/api/fomo/activity")).json()["coins"]) == 2
