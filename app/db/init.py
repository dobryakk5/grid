...
def schema_fingerprint() -> str:
    """A hash of everything the DDL below would do, to tell whether it must.

    Every table, column, key and index as the models declare them, plus every
    additive migration. Any change to either -- a new model, a new column, a
    new statement -- changes the hash; nothing else does.

    Built from the model description rather than from compiled ``CREATE
    TABLE`` text on purpose: ``create_all`` resolves the foreign-key cycle
    between ``grid_profiles`` and ``grid_ranges`` by marking those keys
    ``use_alter``, which changes the compiled text from then on. A hash of
    that text differs before and after the first migration in a process,
    and would re-run the DDL it exists to skip.
    """
    ...
    for table in sorted(Base.metadata.tables.values(), key=lambda t: t.name):
        ...
        for column in table.columns:
            ...
            parts.append(
                ...
                f" null={column.nullable} pk={column.primary_key} default={default}"
            )
        for key in sorted(table.foreign_keys, key=lambda k: (k.parent.name, k.target_fullname)):
            parts.append(f"  fk {key.parent.name} -> {key.target_fullname} ondelete={key.ondelete}")
        for constraint in sorted(
            (c for c in table.constraints if isinstance(c, UniqueConstraint)),
            key=lambda c: [col.name for col in c.columns],
        ):
            parts.append(f"  unique {[col.name for col in constraint.columns]}")
        parts.extend(sorted(
            f"  {CreateIndex(index).compile(dialect=dialect)}" for index in table.indexes
        ))
    parts.extend(_MIGRATIONS)
    return hashlib.sha256("\n".join(parts).encode()).hexdigest()


async def _schema_is_current(fingerprint: str) -> bool:
    async with engine.connect() as conn:
        if await conn.scalar(text("SELECT to_regclass('schema_state')")) is None:
            return False
        applied = await conn.scalar(text("SELECT fingerprint FROM schema_state WHERE id = 1"))
    return applied == fingerprint


async def _migrate(fingerprint: str) -> None:
    async with engine.begin() as conn:
        ...
        await conn.execute(text(
            "INSERT INTO schema_state (id, fingerprint) VALUES (1, :f)"
            " ON CONFLICT (id) DO UPDATE SET fingerprint = EXCLUDED.fingerprint, applied_at = now()"
        ), {"f": fingerprint})


async def _init_db() -> None:
    # The DDL only runs when the schema has actually changed. Each
    # ``ALTER TABLE ... IF NOT EXISTS`` takes an ACCESS EXCLUSIVE lock before
    # it discovers there is nothing to do, and ~forty of them in one
    # transaction deadlock against any service already trading -- which,
    # with starts serialised by the lock above, is every service but the
    # first. Skipping a no-op migration is what keeps a restart harmless.
    fingerprint = schema_fingerprint()
    if not await _schema_is_current(fingerprint):
        await _migrate(fingerprint)
