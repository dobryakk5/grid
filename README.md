...
**Что для движка значит STOP на уже подписанном уровне.** Профиль можно
выключить в любой момент, но подписанную транзакцию это не отменяет — она
всё равно либо исполнится, либо упадёт сама. `GridOrder` такого уровня
переходит в `CancelRefused`: остаётся единственным открытым BUY (второй
уровень выше него не взводится, пока этот не разрешится), но повторных
попыток отмены больше нет — venue уже ответил один раз, и отказ не
переигрывается на каждом тике. `sync_open_orders` продолжает опрашивать его
как обычно, и когда intent доходит до `FILLED`, исполнение и `PositionLot`
записываются как всегда. Разница только в том, что `process_filled_orders`
идёт по циклу **после** проверки `profile.enabled` — у выключенного
профиля replacement SELL для этого fill не создаётся. Купленный токен
остаётся открытым лотом, которым дальше распоряжается оператор вручную; это
и есть честная семантика STOP на on-chain венью — «больше никаких новых
действий», а не «транзакция отменена».
...
Схема БД на свежей базе создаётся `make db-init` — API и воркеры делают это
сами при старте, а ручной скрипт нет: он отказывается торговать, а не меняет
базу побочным эффектом от покупки.
...
На `/fomo` — анализатор сделок FOMO Smart Money: кто из ранжированных трейдеров
fomo.family держит и торгует интересующей монетой на Robinhood Chain. Подробности
ниже, в разделе [FOMO Smart Money](#fomo-smart-money-fomo).
...
## FOMO Smart Money (/fomo)

Показывает, кто из ранжированных трейдеров [fomo.family](https://fomo.family) держит
и торгует конкретной монетой на Robinhood Chain — с намерением ответить на вопрос
«стоит ли эта монета внимания», глядя не только на свой собственный грид.

**Источники правды разделены сознательно:**

- **FOMO** отвечает на вопрос «кто этот кошелёк» — идентичность (хэндл, ранг в
  лидерборде) через `/v2/leaderboard` и `/hodlers/top`;
- **сам блокчейн** отвечает на вопрос «что этот кошелёк сделал» — BUY/SELL
  реконструируются из ERC-20 `Transfer`-логов напрямую, а не из `/trades` FOMO.

Так надёжнее: FOMO — это внутренний, не публичный API (реверс-инжинирен из
собственного веб-клиента fomo.family сторонними инструментами, не документирован),
и его Terms прямо запрещают неавторизованный automated scraping. Торговая история,
построенная только на нём, исчезла бы вместе с любым изменением его формата. Если
`/trades` завтра поменяется или откажет, лента сделок на `/fomo` продолжает
работать как ни в чём не бывало — она читает `chain_swaps`, а не FOMO.

Основной рынок PONS/USDG — пул Uniswap v4, где нет отдельного контракта пула
(все свопы идут через общий PoolManager), поэтому классификация "контрагент = пул
⇒ это своп" здесь не работает. Вместо этого сделка определяется по чистой дельте
самого кошелька за транзакцию: `base` пришёл и `quote` ушёл — BUY,
наоборот — SELL. Работает одинаково для v2/v3/v4 и любого роутера, без единого
адреса пула.

**Как получить JWT для FOMO** (не обязательно для базовой работы страницы — лента
сделок и объёмы читаются из своей БД в любом случае; без сессии пропадают только
хэндлы и ранги):

```text
Chrome DevTools → Network → открыть fomo.family
→ найти запрос к prod-api.fomo.family → Request Headers
→ Authorization: Bearer eyJ...
```

Сохранить только часть после `Bearer `. Вставить в поле на странице `/fomo` — токен
живёт в памяти API-процесса, не на диске, и не возвращается обратно в браузер;
либо положить в `.env` как `FOMO_JWT` для воркеров. Сессия FOMO — Privy-токен на
~1 час; страница явно показывает, когда она истекла, и работает без неё.

**Два фоновых воркера** (`make fomo-registry`, `make chain-tape`):

- `fomo_registry` каждые 15 минут опрашивает лидерборд и холдеров, обновляя
  `fomo_traders` (связь FOMO-личность → EVM-кошелёк) и историю рангов
  `fomo_trader_ranks` — ранг #5 сегодня ничего не говорит о ранге #5 месяц назад;
- `chain_tape` каждые 5 секунд сканирует Robinhood Chain на предмет сделок
  отслеживаемых кошельков. Для только что обнаруженного кошелька сначала
  выполняется **backfill** — иначе именно та покупка, из-за которой кошелёк попал
  в реестр, осталась бы незаписанной: реестр узнаёт о кошельке уже постфактум, когда
  общий курсор сканирования уже ушёл вперёд.

Цена сделки: если тихая нога — USDG (или другой символ из `USD_QUOTE_SYMBOLS`), она
и есть доллары почти без погрешности (`pricing_source=QUOTE_LEG`) — точнее любой
соседней свечи. Иначе цена берётся из уже собранных `market_candles`
(`pricing_source=MARKET_PRICE`), а если и там пусто — сделка остаётся `UNPRICED`, не
выдуманной цифрой.

Сырые `Transfer`-логи каждой транзакции хранятся отдельно в `chain_transactions` —
если классификация окажется неверной для какого-то сценария Universal Router,
`scripts/rebuild-chain-swaps.py` пересчитывает `chain_swaps` заново без единого
обращения к RPC.

Перед тем как полагаться на реалтайм-сканирование в проде, стоит прогнать
`make chain-tape-bench` — он меряет на реальном RPC, какая доля прочитанных
`Transfer`-логов вообще относится к отслеживаемым кошелькам (сканирование сейчас
фильтруется по адресам токенов, а не кошельков — дёшево при любом числе кошельков,
но может читать много лишнего на людном рынке).

**Позиции, PnL, обогащение из FOMO (followers/feed) и WebSocket-подписка на
`trading_activity`** — сознательно не в этой версии: сначала должна накопиться
собственная история сделок. Score вида "84/100" не публикуется, пока формула не
...
## Безопасность ключа торгового кошелька

Приватный ключ бот-кошелька лежит на сервере — иначе воркер не сможет подписывать
без человека. Отсюда всё остальное:

- ключ основного кошелька на сервер не попадает **никогда**; бот-кошелёк
  отдельный, и на нём держится только то, что не жалко потерять;
- ключ не пересылается: ни в чат, ни в тикет, ни в мессенджер, ни в шаренный
  терминал. Ключ, который кто-то однажды увидел, считается потраченным —
  единственное лечение — новый кошелёк, потому что старый можно опустошить;
- поэтому ключ генерируется на самой машине и не печатается:

```bash
scripts/dex_new_wallet.py
```

Скрипт создаёт кошелёк, пишет `RH_PRIVATE_KEY` прямо в `.env` с правами 0600 и
показывает только адрес. Адрес — публичная часть, его можно пересылать и класть
в explorer. Повторный запуск без `--force` откажется затирать существующий ключ.
