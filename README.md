...
`fomo_activity_legs` и `fomo_leaderboard_state`. Команда проверяет новый API
`/api/fomo/activity` до открытия браузера. Порт 9000 должен вести к API
...
После входа сборщик получает `/v2/leaderboard/30d?limit=30`, затем
`/v2/users/{id}/swaps` каждого участника. `/trades` содержит сводки позиций,
...
**Покрытие:** пока читается первая доступная страница swaps каждого трейдера.
Если `hasNextPage=true`, история заведомо неполная; если признака нет, полнота
неизвестна. Неподтверждённые параметры пагинации не подставляются. Страница
показывает эти ограничения, пропущенные записи и недостающие места рейтинга.
Период «30 дней» не обещает полный архив за 30 дней. Новые проходы дополняют
локальную историю, повторные swaps не удваивают суммы; между проходами всё
ещё возможны пропуски при высокой активности.
